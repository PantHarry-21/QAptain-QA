...
export default function SignUpPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        router.push('/login?message=Signup successful! Please check your email to activate your account.')
      } else {
      ...
      <main className="relative z-10 flex flex-1 flex-col items-center justify-center px-6 py-12">
        <Card className="w-full max-w-md border-slate-800 bg-slate-900/60 backdrop-blur-xl shadow-2xl shadow-violet-900/20">
          <CardHeader className="space-y-1 pb-6">
            ...
            <p className="text-sm text-slate-400">Join QAPtain to automate your quality engineering</p>
          </CardHeader>
          <CardContent>
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  ...
                  <Input id="firstName" value={firstName} onChange={(e) => setFirstName(e.target.value)} required disabled={isLoading} className="border-slate-700 bg-slate-950/50 text-white focus-visible:ring-violet-500" />
                </div>
                <div className="space-y-2">
                  <Label htmlFor="lastName" className="text-slate-300">Last Name</Label>
                  <Input id="lastName" value={lastName} onChange={(e) => setLastName(e.target.value)} required disabled={isLoading} className="border-slate-700 bg-slate-950/50 text-white focus-visible:ring-violet-500" />
                </div>
              </div>
              <div className="space-y-2">
